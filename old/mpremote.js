...
exports.detectBoardInfo = detectBoardInfo;
...
const MpRemoteManager_1 = require("./MpRemoteManager");
...
const debugLog = (...args) => {
    try {
        const enabled = vscode.workspace.getConfiguration().get("microPythonWorkBench.debug", false);
        if (enabled)
            console.debug(...args);
    }
    catch { }
};
async function formatMpremoteCmd(args, pythonPath) {
    const escaped = args.map(arg => arg.includes(' ') ? `"${arg.replace(/"/g, '\\"')}"` : arg).join(' ');
    const py = pythonPath ?? await MpRemoteManager_1.MpRemoteManager.detectPythonPath();
    if (py)
        return `"${py}" -m mpremote ${escaped}`;
...
function getEffectiveDeviceRootSync() {
    try {
...
function toLocalRelative(devicePath, rootPath) {
    ...
    if (normRoot === "/")
        return dp; // treat '/' as full device root mapping
...
class ConnectionManager {
    constructor() {
        ...
        this.HEALTH_CHECK_INTERVAL = 5000; // 5 seconds
    ...
    startHealthChecks() {
        ...
        try {
            if (this.healthCheckTimer && this.healthCheckTimer.unref)
...
async function detectPythonPath() {
    ...
    if (configuredPath) {
        try {
            await new Promise((resolve, reject) => {
                (0, node_child_process_1.execFile)(configuredPath, ['--version'], (error) => {
                    if (error)
                        reject(error);
                    else
                        resolve();
    ...
    for (const candidate of candidates) {
        try {
            await new Promise((resolve, reject) => {
                (0, node_child_process_1.execFile)(candidate, ['--version'], (error) => {
                    if (error)
                        reject(error);
                    else
                        resolve();
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const executeCommand = async () => {
            ...
            try {
                ...
                const res = await MpRemoteManager_1.MpRemoteManager.run(args, { cwd: opts.cwd, env, retryOnFailure: opts.retryOnFailure, pythonPath: opts.pythonPath, timeoutMs: opts.timeoutMs });
                if (port) {
                    connectionManager.markHealthy(port);
                ...
                if (port)
                    connectionManager.markOperationCompleted(port);
                if (attempt <= maxRetries && (errorStr.includes("device not configured") ||
                    ...
                    if (errorStr.includes("it may be in use by another program") || errorStr.includes("failed to access")) {
                        try {
                            ...
                            try {
                                MpRemoteManager_1.MpRemoteManager.cancelActive();
                            }
                            catch (e) {
                                console.warn('[DEBUG] runMpremote: cancelActive failed', e);
                            }
...
async function populateFileTreeCache() {
    try {
        ...
        const connectSetting = vscode.workspace.getConfiguration().get("microPythonWorkBench.connect", "auto") || "auto";
        ...
        if (connectSetting === "auto" && MpRemoteManager_1.MpRemoteManager.getActiveConnectionPort() === null) {
...
function getEntriesFromCache(targetPath) {
    if (!globalFileTreeCache) {
...
async function debugTreeParsing() {
    try {
        const connect = normalizeConnect(vscode.workspace.getConfiguration().get("microPythonWorkBench.connect", "auto") || "auto");
...
function parseTreeLines(treeOutput) {
    ...
    for (const line of lines) {
        ...
        if (!line.trim() || line.includes('tree') || line === ':/' || line === ':')
            continue;
...
function determineIsDirectory(name, allLines, currentIndex) {
    ...
    for (let j = currentIndex + 1; j < allLines.length && j < currentIndex + 10; j++) {
        ...
        if (!nextLine || nextLine.includes('tree') || nextLine === ':')
            break;
        // Parse the next line
...
function buildTreeFromParsedLines(parsedLines) {
...
function parseTreeForPath(treeOutput, targetPath) {
...
async function lsTyped(p) {
    ...
    try {
        ...
        if (!cacheValid) {
            ...
            const allowOnActivate = vscode.workspace.getConfiguration().get("microPythonWorkBench.connectOnActivate", false);
        ...
        if (result.length === 0) {
            ...
            try {
                ...
                for (const line of lines) {
                    ...
                    if (!trimmed || trimmed.includes('ls'))
                        continue;
...
async function deleteDirectoryRecursive(p, connect) {
    ...
    try {
        ...
        for (const line of lines) {
            try {
                ...
                if (!trimmed || trimmed.includes('tree'))
                    continue;
...
async function healthCheck(port) {
    ...
    const connect = port || normalizeConnect(vscode.workspace.getConfiguration().get("microPythonWorkBench.connect", "auto") || "auto");
    try {
...
async function mvOnDevice(src, dst) {
    ...
    try {
        const normalizePath = (p) => {
            ...
            if (!trimmed)
                throw new Error("Invalid path for mv");
