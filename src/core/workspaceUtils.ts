...
export function getLocalSyncRoot(workspaceFolder?: vscode.WorkspaceFolder): string {
  const ws = workspaceFolder ?? vscode.workspace.workspaceFolders?.[0];
  if (!ws) throw new Error('No workspace folder open');
  const syncLocalRoot = vscode.workspace.getConfiguration(undefined, ws.uri).get<string>('microPythonWorkBench.syncLocalRoot', '');
