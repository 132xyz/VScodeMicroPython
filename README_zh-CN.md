...
- `MicroPython 工作台: 刷新`: 只读取配置 `rootPath` 的直接子项, 展开目录时按需读取其子项. 浏览文件树不再递归扫描整个设备或 SD 卡.
- 文件树标题栏的上传按钮将文件/文件夹上传到设备根目录 `/`, `...` 菜单还提供根目录新建文件/文件夹. 这些专用根命令不跟随当前选中项或 `rootPath`. 右键文件夹可上传到该目录,右键文件可上传到其父目录.`Board (/)` 根节点提供上传、新建和刷新,不显示删除或重命名. 无需依赖空白区域的右键菜单.
...
- 单文件本地到开发板同步、文件/文件夹上传成功后,状态栏提示 3 秒后自动消失. 错误和警告仍保留通知.
