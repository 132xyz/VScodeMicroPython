...
class ReplSessionSymbols:
    ...
    def has_runtime_root(self, expression: str) -> bool:
        """Return whether an expression root exists in the current REPL session.

        :param expression: Prompt expression before the final attribute prefix.
        :return: True when the root was introduced by successful user source.
        """
        root, _ = _split_expression(expression)
        return root in self._names
