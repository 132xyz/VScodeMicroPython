...
from mpyrepl.completion import engine as completion_engine_module
from mpyrepl.completion import fallbacks as completion_fallbacks_module
from mpyrepl.completion import parser as completion_parser_module
from mpyrepl.completion import state as completion_state_module
from mpyrepl.completion import stubs as completion_stubs_module
from mpyrepl.repl import session as session_module
from mpyrepl.runtime import transport as transport_module
from mpyrepl.cli import build_parser
from mpyrepl.completion.engine import (
    ReplCompleter,
    cursor_in_string_or_comment,
    discover_stub_modules,
    parse_completion_request,
)
from mpyrepl.completion.device import (
...
from mpyrepl.completion.fallbacks import fallback_candidates_for_expression
from mpyrepl.completion.state import ReplSessionSymbols
from mpyrepl.completion.state import _display_meta_for_kind
from mpyrepl.completion.stubs import StubCompletionIndex
from mpyrepl.repl.control import ControlRequest, FileControlChannel
from mpyrepl.repl.indent import (
    continuation_default,
    is_block_complete,
    leading_indent_prefix,
    should_start_multiline,
)
from mpyrepl.repl.lexer import MicroPythonLexer, _append_style, build_repl_style
...
class SupportModuleTests(unittest.TestCase):
    def test_bootstrap_configure_import_path_adds_scripts_and_vendor_dirs(self) -> None:
        package_dir = os.path.dirname(os.path.abspath(bootstrap.__file__))
        ...
        with mock.patch.object(bootstrap.sys, "path", ["existing"]):
            ...
            self.assertEqual(bootstrap.sys.path[1], scripts_dir)
...
""".strip(),
    ...
    def test_completion_engine_uses_common_module_fallbacks_without_stub_or_device(self) -> None:
        ...
        self.assertEqual(calls, [("lv", "a"), ("lvgl", "device")])
...
""".strip(),
            ...
            self.assertEqual(calls, [("hw", "P"), ("pin", "v")])
            ...
            self.assertEqual(calls, [("hw", "P"), ("pin", "v"), ("machine", "Z")])
            ...
            self.assertEqual(
                calls,
                [("hw", "P"), ("pin", "v"), ("machine", "Z"), ("dynamic", "Z")],
            )

    def test_tracked_module_completion_merges_device_members_missing_from_stub(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            stub_root = Path(tmp_dir)
            (stub_root / "esp32.pyi").write_text(
                "def idf_heap_info(capabilities: int): ...\n",
                encoding="utf-8",
            )
            calls: list[tuple[str, str]] = []
            ...
            def dotted_provider(expression: str, prefix: str):
                calls.append((expression, prefix))
                return ["idf_heap_info", "idf_task_info"]

            symbols = ReplSessionSymbols()
            symbols.record_successful_source("import esp32")
            completer = ReplCompleter(
                symbols,
                stub_root=str(stub_root),
                dotted_provider=dotted_provider,
            ...
            items = list(
                completer.get_completions(
                    Document("esp32.idf_", cursor_position=len("esp32.idf_")),
                    CompleteEvent(text_inserted=True),
                )
            )
            cached_items = list(
                completer.get_completions(
                    Document("esp32.idf_t", cursor_position=len("esp32.idf_t")),
                    CompleteEvent(text_inserted=True),
                )
            )

            self.assertEqual([item.text for item in items], ["idf_heap_info", "idf_task_info"])
            self.assertEqual(
                [item.display_meta_text for item in items],
                ["stub function (capabilities: int)", "device"],
            )
            self.assertEqual([item.text for item in cached_items], ["idf_task_info"])
            self.assertEqual(calls, [("esp32", "idf_")])
...
""".strip()
    ...
    def test_utf8_stream_decoder_handles_split_code_points(self) -> None:
        ...
        from mpyrepl.runtime import decode as decode_module
    ...
    def test_models_exec_result_and_repl_config_hold_values(self) -> None:
        from mpyrepl.runtime import models as models_module
