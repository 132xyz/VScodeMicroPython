...
from mpyrepl import app as mpyrepl_app
from mpyrepl.repl import async_runner as mpyrepl_main
...
class MainHelperTests(unittest.TestCase):
    ...
    def test_run_ports_emits_success_and_error_payloads(self) -> None:
        ...
        with mock.patch.object(mpyrepl_app, "list_serial_ports", return_value=[{"port": "COM7"}]):
            with mock.patch.object(mpyrepl_app.sys, "stdout", stdout_stream):
                self.assertEqual(mpyrepl_app.run_ports(), 0)
        ...
        stdout_stream = io.StringIO()
        with mock.patch.object(mpyrepl_app, "list_serial_ports", side_effect=RuntimeError("scan failed")):
            with mock.patch.object(mpyrepl_app.sys, "stdout", stdout_stream):
                self.assertEqual(mpyrepl_app.run_ports(), 1)
    ...
    def test_run_fs_cli_uses_custom_transport_and_maps_errors(self) -> None:
        ...
        with mock.patch.object(mpyrepl_app, "SerialReplTransport", FakeContextTransport):
            with mock.patch.object(mpyrepl_app, "DeviceFsClient") as device_fs_client:
                with mock.patch.object(mpyrepl_app, "run_fs_operation", return_value={"exists": True}):
                    with mock.patch.object(mpyrepl_app.sys, "stdout", stdout_stream):
                        self.assertEqual(mpyrepl_app.run_fs_cli(config, args), 0)
        ...
        for error, expected_code in (
            (mpyrepl_app.FsOperationError("missing", "missing"), "missing"),
            (RuntimeError("boom"), "error"),
        ):
            stdout_stream = io.StringIO()
            with mock.patch.object(mpyrepl_app, "SerialReplTransport", FakeContextTransport):
                with mock.patch.object(mpyrepl_app, "DeviceFsClient"):
                    with mock.patch.object(mpyrepl_app, "run_fs_operation", side_effect=error):
                        with mock.patch.object(mpyrepl_app.sys, "stdout", stdout_stream):
                            self.assertEqual(mpyrepl_app.run_fs_cli(config, args), 1)
            payload = json.loads(stdout_stream.getvalue())
            self.assertFalse(payload["ok"])
    ...
    def test_run_fs_cli_write_file_progress_adds_callback(self) -> None:
        ...
        with mock.patch.object(mpyrepl_app, "SerialReplTransport", FakeContextTransport):
            with mock.patch.object(mpyrepl_app, "DeviceFsClient"):
                with mock.patch.object(mpyrepl_app, "run_fs_operation", side_effect=fake_run_fs_operation):
                    with mock.patch.object(mpyrepl_app.sys, "stdout", stdout_stream):
                        self.assertEqual(mpyrepl_app.run_fs_cli(config, args), 0)
        ...
        self.assertTrue(stdout_lines[0].startswith(mpyrepl_app.PROGRESS_MARKER))
    ...
    def test_main_dispatches_exec_and_maps_errors_to_exit_codes(self) -> None:
        ...
        with mock.patch.object(mpyrepl_app, "ensure_python_version"):
            with mock.patch.object(mpyrepl_app, "parse_args", return_value=args):
                with mock.patch.object(mpyrepl_app, "run_exec", return_value=7) as run_exec:
                    self.assertEqual(mpyrepl_app.main(), 7)

        self.assertTrue(hasattr(mpyrepl_app, "run_async_repl"))
        ...
        with mock.patch.object(mpyrepl_app, "ensure_python_version"):
            with mock.patch.object(mpyrepl_app, "parse_args", return_value=args):
                with mock.patch.object(
                    ...
                    with mock.patch.object(mpyrepl_app.sys, "stderr", stderr_stream):
                        self.assertEqual(mpyrepl_app.main(), 2)
        ...
        with mock.patch.object(mpyrepl_app, "ensure_python_version"):
            with mock.patch.object(mpyrepl_app, "parse_args", return_value=args):
                with mock.patch.object(
                    ...
                    with mock.patch.object(mpyrepl_app.sys, "stderr", stderr_stream):
                        self.assertEqual(mpyrepl_app.main(), 3)
    ...
    def test_main_dispatches_other_commands(self) -> None:
        ...
        for command, function_name, status in dispatch_cases:
            ...
            with mock.patch.object(mpyrepl_app, "ensure_python_version"):
                with mock.patch.object(mpyrepl_app, "parse_args", return_value=args):
                    with mock.patch.object(mpyrepl_app, function_name, return_value=status):
                        self.assertEqual(mpyrepl_app.main(), status)
        ...
        with mock.patch.object(mpyrepl_app, "ensure_python_version"):
            with mock.patch.object(mpyrepl_app, "parse_args", return_value=args):
                with mock.patch.object(
                    ...
                    with mock.patch.object(mpyrepl_app.asyncio, "run", return_value=24) as asyncio_run:
        ...
        with mock.patch.object(mpyrepl_app, "ensure_python_version"):
            with mock.patch.object(mpyrepl_app, "parse_args", return_value=args):
                with mock.patch.object(mpyrepl_app, "run_ports", return_value=27):
                    self.assertEqual(mpyrepl_app.main(), 27)
