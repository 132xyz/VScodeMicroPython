...
自动生成的右符号会在输入过程中持续跟踪位置.输入对应右符号时会越过该自动符号,
光标位于空符号对中间时按 Backspace 会同时删除两侧.历史记录或粘贴文本中原本存在的
右符号不会被误认为自动符号.选中文本后输入左符号或引号会包裹选区,字符串和注释中的
括号则保持普通字符输入.
...
扩展对同一设备的并发启动请求共享同一次启动,启动、关闭和切换设备按顺序处理,子进程诊断只属于对应启动尝试.raw-paste 上传失败后会尝试有界的 Ctrl-C 中止并读取协议结束响应,不会自动重发源码或软复位.发送下一块源码前先消费已到达的流控和提前终止响应,串口短写只按实际接收字节数推进.

文件浏览使用按设备和目录隔离的直接列表缓存,有效期 30 秒,空目录同样缓存.同一目录的并发请求共享一次读取.刷新使缓存失效并读取配置的根目录,展开的目录各自加载子项.刷新前的迟到响应不能覆盖当前缓存.浏览过程不再生成递归 `tree-paths.json`;全量同步和显式 Agent `tree` 保持递归行为.
...
人工 REPL 使用持续事件循环和有序输出呈现器.完整行进入终端历史,未完成行显示在正在编辑的提示符上方.CRLF 即使跨事件仍只算一次换行,单独 CR 更新当前输出行.两次提示符之间直接显示输出;进入新提示符前会给尚未换行的输出增加显示分隔,不修改 RPC 返回的原始文本.编辑时无换行尾行超过 65,536 个字符会分成显示行,不丢字符.设备 ANSI 转义序列不能改变宿主终端模式.

自动补全和 Tab 使用异步请求,回复预算 500ms,过期结果不会覆盖新输入.补全不会自行连接或恢复失步设备.其他客户端执行代码时,输入和输出绘制仍可响应.manager 输出序号用于发现缺口.慢 socket 客户端的待发送预算为 4 MiB,超过后带缺口诊断断开;终端待绘制预算为 1,048,576 个字符,超限会明确提示.这些边界避免静默丢失和无限内存增长,但不能保证阻塞消费者永久保留无限输出.

文件系统和设备补全的内部响应使用每次操作独立 nonce 和编码字节长度的帧.只消费匹配记录,前后后台打印继续转发到共享控制台.raw 提示符就绪状态与普通接收字节分开,空闲读取不再删除正文 `>`.若其他线程把字节插入内部记录,解析失败会报告协议错误,不会猜测修补后继续.
...
串口管理器会在协议操作空闲时检测设备连接状态。USB 串口被拔出或驱动返回 `ReadFile`、`WriteFile`、`ClearCommError` 错误时, 扩展会把串口状态更新为断开, 但保留 REPL 终端用于查看诊断信息。重新插入设备后, 需要重新执行打开串口或打开 REPL 操作。

如果主机侧 REPL 客户端自身崩溃或以非零状态退出, VS Code 会保留终端, 而不是自动关闭。终端里应能看到 Python traceback 和一行简短的 `mpyrepl` 诊断信息。

串口传输丢失、manager 退出和自动恢复失败等非用户异常也会保留 REPL 终端现场.
扩展只有在用户明确执行关闭/停止、切换或重新连接串口时才会销毁终端.
VS Code 窗口或应用退出期间的终端生命周期仍由 VS Code 自身控制.
