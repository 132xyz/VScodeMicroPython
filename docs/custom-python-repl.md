...
Concurrent extension startup requests for the same device share one pending startup. Start, stop, and device-switch operations are serialized, and child-process diagnostics remain scoped to that startup attempt. A failed raw-paste upload attempts a bounded Ctrl-C abort and consumes the ending protocol response; it does not automatically resend the source or soft-reset the board. Pending flow-control and early-termination responses are processed before sending more source, and short writes advance only by the accepted byte count.

The file browser uses direct directory listings, cached by device and directory for 30 seconds, including empty results. Duplicate requests share the same pending read. Refresh invalidates listings and reads the configured root; expanded directories load their own children. Late pre-refresh responses cannot overwrite the current cache. Browsing does not generate a recursive `tree-paths.json`; full sync and explicit Agent `tree` retain their recursive behavior.
...
The human REPL uses one persistent event loop and an ordered console presenter. Completed lines go to scrollback; an unfinished line is shown above the editable prompt. CRLF remains one newline even across events, and a bare CR updates the current output line. Between prompts, output streams directly. Before a new prompt, an open output line receives a display-only line break; RPC output bytes are unchanged. A newline-free editing tail is split into display lines after 65,536 characters without discarding characters. ANSI escape sequences cannot change host terminal modes.

Automatic completion and Tab use asynchronous requests with a 500ms reply budget; outdated results are ignored. Completion does not connect or recover an unsynchronized device. The console remains responsive while another client executes code. Manager event sequence numbers expose gaps. A slow socket consumer has a 4 MiB pending-output budget and is disconnected with an output-gap diagnostic; the terminal has a separate 1,048,576-character pending-render budget and visibly reports overflow. These limits avoid silent loss or unbounded memory growth, but cannot guarantee unlimited output retention for a stalled consumer.

Internal filesystem and completion replies use per-operation framed responses with a nonce and encoded-byte length. Only matching records are consumed; surrounding background output is forwarded to the shared console. Raw prompt readiness is stored independently of ordinary received text, so idle output does not delete a `>` character. Bytes interleaved inside an internal record cause a protocol error rather than guessed reconstruction.
